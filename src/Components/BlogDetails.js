...
const BlogDetails = () => {
    ...
    const fetchBlog = async () => {
        try {
            ...
            if (response?.data?.length > 0) {
                ...
                const currentCategoryId = currentBlog.category?.id;
    ...
    const getAuthors = () => {
        const authors = [];
        if (blog.author?.name) authors.push(blog.author.name);
        if (blog.authors_2?.length > 0) {
        ...
        <div className="container mx-auto px-4 py-8">
            <div className="max-w-4xl mx-auto" data-aos="fade-up">
                ...
                <div className="prose lg:prose-xl mx-auto text-gray-800 mb-6">
                    {blog?.Body?.map((block) => renderDynamicZone(block))}
                </div>
            </div>
        </div>
    );
